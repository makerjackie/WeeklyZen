...
export default function MeditationPage() {
  ...
  const [selectedSound, setSelectedSound] = useState<SoundData | null>(
    defaultSound
  )
  ...
  const handleTimerEnd = () => {
    ...
    const playEndSound = async () => {
      const endSoundUrl =
        'https://objectstorageapi.gzg.sealos.run/e36y8btp-weeklyzen/audio/bg/temple-bells.mp3'
      ...
      // 设置调试输出，帮助跟踪音频元素状态
      endSoundRef.current.onloadeddata = () =>
        console.log('[调试] 结束音效音频数据加载完成')
      ...
      const attemptPlaySound = async () => {
        try {
          ...
          try {
            const buffer = await audioManager.current.loadAudioBuffer(
              endSoundUrl,
              'end-sound'
            )
            ...
            if (endSoundRef.current) {
              ...
              return new Promise<boolean>((resolve) => {
                ...
                const onCanPlay = async () => {
                  // 移除事件监听器
                  if (endSoundRef.current) {
                    endSoundRef.current.removeEventListener(
                      'canplaythrough',
                      onCanPlay
                    )
                  }
                ...
                if (endSoundRef.current) {
                  ...
                  endSoundRef.current.addEventListener(
                    'canplaythrough',
                    onCanPlay,
                    { once: true }
                  )
      ...
      try {
        while (playAttempts < maxAttempts) {
          ...
          playAttempts++
          console.log(
            `[调试] 播放尝试 ${playAttempts}/${maxAttempts} 失败，${playAttempts < maxAttempts ? '正在重试...' : '已达到最大重试次数'}`
          )
          // 重试前等待短暂时间
    ...
    if (guidanceAudio && selectedGuidance?.audioUrl) {
      ...
      if (
        (selectedGuidance.id === 'custom-guidance' ||
    ...
    if (newPlayingState) {
      ...
      if (selectedSound && audioRef.current) {
        ...
        if (
          ...
          !audioRef.current.src.includes(selectedSound.id)
        ) {
  ...
  const handleDurationSelect = (duration: number) => {
    ...
    if (guidanceAudio && selectedGuidance) {
      ...
      if (selectedGuidance.audioUrl) {
        console.log(
          '[调试] 创建新的引导语音频实例以重置播放位置:',
          selectedGuidance.audioUrl
        )
        const audio = new Audio(selectedGuidance.audioUrl)
        audio.volume = isMuted ? 0 : volume / 100
  ...
  const handleShowDurationMenu = (open: boolean) => {
    if (!isPlaying) {
      ...
      toast.info(
        t(
          '请先暂停冥想后再更改时长',
          'Please pause meditation before changing duration'
        )
      )
    }
  }
  ...
  const handleCustomAudioGenerated = useCallback(
  ...
  useEffect(() => {
    ...
    if (customAudioUrl && guidanceAudio) {
      console.log(
        '[调试] 检测到customAudioUrl和guidanceAudio都存在，设置播放逻辑'
      )
      ...
      if (
        ...
        guidanceAudio.onended = function () {
          console.log(
            '[调试] useEffect中设置的事件：引导语音频播放结束，播放自定义音频'
          )
    ...
    <div className={`min-h-screen ${bgGradient} ${textColor} flex flex-col`}>
      ...
      <div className="relative flex flex-1 flex-col items-center justify-center">
        ...
        <div className="pointer-events-none absolute inset-0 z-0 flex items-center justify-center">
          <div className="relative h-[80vmin] w-[80vmin] md:max-h-[500px] md:max-w-[500px]">
            <BreathingSphere
              isPlaying={isPlaying}
              showText={false}
              size="medium"
        ...
        <div className="absolute left-1/2 top-1/2 z-10 -translate-x-1/2 -translate-y-1/2 transform">
          <div className="text-center text-4xl font-light tracking-widest md:text-6xl lg:text-7xl">
            {formatTime(timeLeft)}
          </div>
        </div>
        ...
        <div className="relative z-20 mt-[40vh] md:mt-[60vh]">
          <Button
            ...
            className={`h-14 w-14 rounded-full md:h-16 md:w-16 ${
              isDarkTheme
                ? 'border border-blue-600/30 bg-blue-600/20 text-blue-300 hover:bg-blue-600/30'
                : 'border border-blue-200 bg-white/80 text-blue-600 hover:bg-white'
            }`}
      ...
      <Dialog open={showSoundDialog} onOpenChange={setShowSoundDialog}>
        ...
        >
          <DialogHeader>
            <DialogTitle>
              {t('选择背景音效', 'Choose Background Sound')}
            </DialogTitle>
          </DialogHeader>
            ...
            <GuidanceSelector
              ...
              onGuidanceSelect={(guidance) => {
                handleGuidanceSelect({
                  ...guidance,
                  audioUrl: guidance.audioUrl || undefined,
                })
              }}
      ...
      {/* 引导语全文对话框 */}
      <Dialog
        open={showGuidanceTextDialog}
        onOpenChange={setShowGuidanceTextDialog}
      >
        <DialogContent
          className={`${isDarkTheme ? 'bg-slate-900 text-white' : 'bg-white text-slate-800'} mx-auto w-[90vw] max-w-md`}
        >
          <DialogHeader>
            <DialogTitle>
              {selectedGuidance?.title || t('引导语', 'Guidance')}
            </DialogTitle>
          </DialogHeader>
          <div className="max-h-[70vh] overflow-y-auto">
            {selectedGuidance?.paragraphs.map((paragraph, idx) => (
              <p
                key={`paragraph-${selectedGuidance.id}-${idx}`}
